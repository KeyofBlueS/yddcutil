...
display_selection(){
	...
	BUS="$(yad $YCOMMOPT --window-icon "monitor" --image "monitor" --title="Display Selection" --text="Please select a Display

$( echo "$MONITORS")" --selectable-labels --buttons-layout=center --form --field="i2c:CB" "$i2clists" \
--button="Start"!forward:0 \
--button="Help"!help-about-symbolic:97 \
--button="Exit"!exit:99 )"
	display_choice=$?
	BUS="$(echo $BUS | grep -Po '\d+')"
...
image_menu(){
	...
	if [ "$image_choice" -eq 1 ]; then
		...
		ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 1f 03
		image_menu
...
presets_menu(){
	while true; do
		PRESET="$(ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS getvcp dc | awk -F': ' '{print $2}' | awk -F'(' '{print $1}')"
...
Current: $PRESET" $PRESETSBUTTONS --button="Go Back"!back:98
		...
		if [ "$presets_choice" -eq 1 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp dc 00
		elif [ "$presets_choice" -eq 2 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp dc 01
		elif [ "$presets_choice" -eq 3 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp dc 02
		elif [ "$presets_choice" -eq 4 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp dc 03
		elif [ "$presets_choice" -eq 5 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp dc 04
		elif [ "$presets_choice" -eq 6 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp dc 05
		elif [ "$presets_choice" -eq 7 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp dc 06
		elif [ "$presets_choice" -eq 8 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp dc 07
		elif [ "$presets_choice" -eq 9 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp dc 08
		elif [ "$presets_choice" -eq 10 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp dc 09
		elif [ "$presets_choice" -eq 11 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp dc 0a
		elif [ "$presets_choice" -eq 12 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp dc f0
...
set_brightness(){
	while true; do
	BRIGHTNESS="$(ddcutil --sleep-multiplier=$SLEEPMULTIPLIER -t --bus=$BUS getvcp 10 | awk -F' ' '{print $4}')"
...
--button="Go Back"!back:98
	...
	if [ "$brightness_choice" -eq 0 ]; then
		ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 10 0
	elif [ "$brightness_choice" -eq 1 ]; then
		ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 10 10
	elif [ "$brightness_choice" -eq 2 ]; then
		ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 10 20
	elif [ "$brightness_choice" -eq 3 ]; then
		ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 10 30
	elif [ "$brightness_choice" -eq 4 ]; then
		ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 10 40
	elif [ "$brightness_choice" -eq 5 ]; then
		ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 10 50
	elif [ "$brightness_choice" -eq 6 ]; then
		ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 10 60
	elif [ "$brightness_choice" -eq 7 ]; then
		ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 10 70
	elif [ "$brightness_choice" -eq 8 ]; then
		ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 10 80
	elif [ "$brightness_choice" -eq 9 ]; then
		ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 10 90
	elif [ "$brightness_choice" -eq 10 ]; then
		ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 10 100
...
set_contrast(){
	while true; do
	CONTRAST="$(ddcutil --sleep-multiplier=$SLEEPMULTIPLIER -t --bus=$BUS getvcp 12 | awk -F' ' '{print $4}')"
...
--button="Go Back"!back:98
	...
	if [ "$contrast_choice" -eq 0 ]; then
		ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 12 0
	elif [ "$contrast_choice" -eq 1 ]; then
		ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 12 10
	elif [ "$contrast_choice" -eq 2 ]; then
		ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 12 20
	elif [ "$contrast_choice" -eq 3 ]; then
		ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 12 30
	elif [ "$contrast_choice" -eq 4 ]; then
		ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 12 40
	elif [ "$contrast_choice" -eq 5 ]; then
		ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 12 50
	elif [ "$contrast_choice" -eq 6 ]; then
		ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 12 60
	elif [ "$contrast_choice" -eq 7 ]; then
		ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 12 70
	elif [ "$contrast_choice" -eq 8 ]; then
		ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 12 80
	elif [ "$contrast_choice" -eq 9 ]; then
		ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 12 90
	elif [ "$contrast_choice" -eq 10 ]; then
		ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 12 100
...
set_temperature(){
	while true; do
		TEMPERATURE="$(ddcutil --sleep-multiplier=$SLEEPMULTIPLIER -t --bus=$BUS getvcp 14 | awk -F' ' '{print $4}')"
...
Current: $TEMPERATURE" $TEMPERATUREBUTTONS --button="Go Back"!back:98
		...
		if [ "$temperature_choice" -eq 1 ]; then
			if [ "$TEMPERATURE" = "sRGB" ]; then
				...
				ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 14 0x01
			fi
		elif [ "$temperature_choice" -eq 2 ]; then
			if [ "$TEMPERATURE" = "Native" ]; then
				true
			else
				ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 14 0x02
			fi
		elif [ "$temperature_choice" -eq 3 ]; then
			if [ "$TEMPERATURE" = "4000 K" ]; then
				true
			else
				ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 14 0x03
			fi
		elif [ "$temperature_choice" -eq 4 ]; then
			if [ "$TEMPERATURE" = "5000 K" ]; then
				true
			else
				ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 14 0x04
			fi
		elif [ "$temperature_choice" -eq 5 ]; then
			if [ "$TEMPERATURE" = "6500 K" ]; then
				true
			else
				ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 14 0x05
			fi
		elif [ "$temperature_choice" -eq 6 ]; then
			if [ "$TEMPERATURE" = "7500 K" ]; then
				true
			else
				ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 14 0x06
			fi
		elif [ "$temperature_choice" -eq 7 ]; then
			if [ "$TEMPERATURE" = "8200 K" ]; then
				true
			else
				ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 14 0x07
			fi
		elif [ "$temperature_choice" -eq 8 ]; then
			if [ "$TEMPERATURE" = "9300 K" ]; then
				true
			else
				ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 14 0x08
			fi
		elif [ "$temperature_choice" -eq 9 ]; then
			if [ "$TEMPERATURE" = "10000 K" ]; then
				true
			else
				ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 14 0x09
			fi
		elif [ "$temperature_choice" -eq 10 ]; then
			if [ "$TEMPERATURE" = "11500 K" ]; then
				true
			else
				ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 14 0x0a
			fi
		elif [ "$temperature_choice" -eq 11 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 14 0x0b
			color_menu
		elif [ "$temperature_choice" -eq 12 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 14 0x0c
			color_menu
		elif [ "$temperature_choice" -eq 13 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 14 0x0d
			color_menu
...
color_user_red(){
	while true; do
		REDGAIN="$(ddcutil --sleep-multiplier=$SLEEPMULTIPLIER -t --bus=$BUS getvcp 16 | awk -F' ' '{print $4}')"
...
--button="Go Back"!back:98
		...
		if [ "$redgain_choice" -eq 0 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 16 0
		elif [ "$redgain_choice" -eq 1 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 16 10
		elif [ "$redgain_choice" -eq 2 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 16 20
		elif [ "$redgain_choice" -eq 3 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 16 30
		elif [ "$redgain_choice" -eq 4 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 16 40
		elif [ "$redgain_choice" -eq 5 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 16 50
		elif [ "$redgain_choice" -eq 6 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 16 60
		elif [ "$redgain_choice" -eq 7 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 16 70
		elif [ "$redgain_choice" -eq 8 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 16 80
		elif [ "$redgain_choice" -eq 9 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 16 90
		elif [ "$redgain_choice" -eq 10 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 16 100
		elif [ "$redgain_choice" -eq 11 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 16 - 1
...
color_user_green(){
	while true; do
		GREENGAIN="$(ddcutil --sleep-multiplier=$SLEEPMULTIPLIER -t --bus=$BUS getvcp 18 | awk -F' ' '{print $4}')"
...
--button="Go Back"!back:98
		...
		if [ "$greengain_choice" -eq 0 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 18 0
		elif [ "$greengain_choice" -eq 1 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 18 10
		elif [ "$greengain_choice" -eq 2 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 18 20
		elif [ "$greengain_choice" -eq 3 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 18 30
		elif [ "$greengain_choice" -eq 4 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 18 40
		elif [ "$greengain_choice" -eq 5 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 18 50
		elif [ "$greengain_choice" -eq 6 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 18 60
		elif [ "$greengain_choice" -eq 7 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 18 70
		elif [ "$greengain_choice" -eq 8 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 18 80
		elif [ "$greengain_choice" -eq 9 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 18 90
		elif [ "$greengain_choice" -eq 10 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 18 100
		elif [ "$greengain_choice" -eq 11 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 18 - 1
...
color_user_blue(){
	while true; do
		BLUEGAIN="$(ddcutil --sleep-multiplier=$SLEEPMULTIPLIER -t --bus=$BUS getvcp 1a | awk -F' ' '{print $4}')"
...
--button="Go Back"!back:98
		...
		if [ "$bluegain_choice" -eq 0 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 1a 0
		elif [ "$bluegain_choice" -eq 1 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 1a 10
		elif [ "$bluegain_choice" -eq 2 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 1a 20
		elif [ "$bluegain_choice" -eq 3 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 1a 30
		elif [ "$bluegain_choice" -eq 4 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 1a 40
		elif [ "$bluegain_choice" -eq 5 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 1a 50
		elif [ "$bluegain_choice" -eq 6 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 1a 60
		elif [ "$bluegain_choice" -eq 7 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 1a 70
		elif [ "$bluegain_choice" -eq 8 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 1a 80
		elif [ "$bluegain_choice" -eq 9 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 1a 90
		elif [ "$bluegain_choice" -eq 10 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 1a 100
		elif [ "$bluegain_choice" -eq 11 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 1a - 1
...
geometry_menu(){
	...
	if [ "$color_choice" -eq 1 ]; then
		...
		ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 1e 03
		geometry_menu
	elif [ "$color_choice" -eq 98 ]; then
		main_menu
	else
...
geometry_horizontal_position(){
	while true; do
		HORIZONTAL="$(ddcutil --sleep-multiplier=$SLEEPMULTIPLIER -t --bus=$BUS getvcp 20 | awk -F' ' '{print $4}')"
...
--button="Go Back"!back:98
		...
		if [ "$horizontal_choice" -eq 0 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 20 0
		elif [ "$horizontal_choice" -eq 1 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 20 10
		elif [ "$horizontal_choice" -eq 2 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 20 20
		elif [ "$horizontal_choice" -eq 3 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 20 30
		elif [ "$horizontal_choice" -eq 4 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 20 40
		elif [ "$horizontal_choice" -eq 5 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 20 50
		elif [ "$horizontal_choice" -eq 6 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 20 60
		elif [ "$horizontal_choice" -eq 7 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 20 70
		elif [ "$horizontal_choice" -eq 8 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 20 80
		elif [ "$horizontal_choice" -eq 9 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 20 90
		elif [ "$horizontal_choice" -eq 10 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 20 100
		elif [ "$horizontal_choice" -eq 11 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 20 - 1
...
geometry_vertical_position(){
	while true; do
		VERTICAL="$(ddcutil --sleep-multiplier=$SLEEPMULTIPLIER -t --bus=$BUS getvcp 30 | awk -F' ' '{print $4}')"
...
--button="Go Back"!back:98
		...
		if [ "$vertical_choice" -eq 0 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 30 0
		elif [ "$vertical_choice" -eq 1 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 30 10
		elif [ "$vertical_choice" -eq 2 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 30 30
		elif [ "$vertical_choice" -eq 3 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 30 30
		elif [ "$vertical_choice" -eq 4 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 30 40
		elif [ "$vertical_choice" -eq 5 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 30 50
		elif [ "$vertical_choice" -eq 6 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 30 60
		elif [ "$vertical_choice" -eq 7 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 30 70
		elif [ "$vertical_choice" -eq 8 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 30 80
		elif [ "$vertical_choice" -eq 9 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 30 90
		elif [ "$vertical_choice" -eq 10 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 30 100
		elif [ "$vertical_choice" -eq 11 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 30 - 1
...
input_menu(){
	while true ; do
		INPUT="$(ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS getvcp 60 | awk -F': ' '{print $2}' | awk -F'(' '{print $1}')"
...
Current: $INPUT" $INPUTBUTTONS --button="Go Back"!back:98
		...
		if [ "$input_choice" -eq 1 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 60 01
		elif [ "$input_choice" -eq 2 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 60 02
		elif [ "$input_choice" -eq 3 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 60 03
		elif [ "$input_choice" -eq 4 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 60 04
		elif [ "$input_choice" -eq 5 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 60 04
		elif [ "$input_choice" -eq 6 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 60 06
		elif [ "$input_choice" -eq 7 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 60 07
		elif [ "$input_choice" -eq 8 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 60 08
		elif [ "$input_choice" -eq 9 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 60 09
		elif [ "$input_choice" -eq 10 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 60 0a
		elif [ "$input_choice" -eq 11 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 60 0b
		elif [ "$input_choice" -eq 12 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 60 0c
		elif [ "$input_choice" -eq 13 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 60 0d
		elif [ "$input_choice" -eq 14 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 60 0e
		elif [ "$input_choice" -eq 15 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 60 0f
		elif [ "$input_choice" -eq 16 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 60 10
		elif [ "$input_choice" -eq 17 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 60 11
		elif [ "$input_choice" -eq 18 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 60 12
...
audio_menu(){
	...
	if [ "$audio_choice" -eq 1 ]; then
		...
		input_menu
	elif [ "$audio_choice" -eq 3 ]; then
		audio_menu
	elif [ "$audio_choice" -eq 4 ]; then
		geometry_menu
	elif [ "$audio_choice" -eq 5 ]; then
		power_menu
	elif [ "$audio_choice" -eq 6 ]; then
		misc_menu
	elif [ "$audio_choice" -eq 7 ]; then
		info_menu
	elif [ "$audio_choice" -eq 8 ]; then
		reset_menu
...
audio_volume(){
	while true; do
		VOLUME="$(ddcutil --sleep-multiplier=$SLEEPMULTIPLIER -t --bus=$BUS getvcp 62 | awk -F' ' '{print $4}')"
...
--button="Go Back"!back:98
		...
		if [ "$volume_choice" -eq 0 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 62 0
		elif [ "$volume_choice" -eq 1 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 62 10
		elif [ "$volume_choice" -eq 2 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 62 20
		elif [ "$volume_choice" -eq 3 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 62 30
		elif [ "$volume_choice" -eq 4 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 62 40
		elif [ "$volume_choice" -eq 5 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 62 50
		elif [ "$volume_choice" -eq 6 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 62 60
		elif [ "$volume_choice" -eq 7 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 62 70
		elif [ "$volume_choice" -eq 8 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 62 80
		elif [ "$volume_choice" -eq 9 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 62 90
		elif [ "$volume_choice" -eq 10 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 62 100
		elif [ "$volume_choice" -eq 11 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 62 - 1
...
power_mode_menu(){
	while true; do
		POWERMODE="$(ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS getvcp d6 | awk -F'): ' '{print $2}' | awk -F'(' '{print $1}')"
...
Current: $POWERMODE" $POWERMODEBUTTONS --button="Go Back"!back:98
		...
		if [ "$power_mode_choice" -eq 1 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp d6 01
		elif [ "$power_mode_choice" -eq 2 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp d6 02
		elif [ "$power_mode_choice" -eq 3 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp d6 03
		elif [ "$power_mode_choice" -eq 4 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp d6 04
		elif [ "$power_mode_choice" -eq 5 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp d6 05
...
power_auxiliary_menu(){
	while true; do
		...
		if [ "$power_auxiliary_choice" -eq 1 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp d7 01
		elif [ "$power_auxiliary_choice" -eq 2 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp d7 02
		elif [ "$power_mode_choice" -eq 98 ]; then
			power_menu
		else
			exit 0
...
info_menu(){
	HORIZONTALFREQ="Horizontal Frequency: $(ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS getvcp ac | awk -F': ' '{print $2}')"
	VERTICALFREQ="Vertical Frequency: $(ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS getvcp ae | awk -F': ' '{print $2}')"
	INPUT="Input Source: $(ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS getvcp 60 | awk -F': ' '{print $2}' | awk -F'(' '{print $1}')"
	BRIGHTNESS="Brightness: $(ddcutil --sleep-multiplier=$SLEEPMULTIPLIER -t --bus=$BUS getvcp 10 | awk -F' ' '{print $4}')"
	CONTRAST="Contrast: $(ddcutil --sleep-multiplier=$SLEEPMULTIPLIER -t --bus=$BUS getvcp 12 | awk -F' ' '{print $4}')"
	TEMPERATURE="$(ddcutil --sleep-multiplier=$SLEEPMULTIPLIER -t --bus=$BUS getvcp 14 | awk -F' ' '{print $4}')"
	if [ "$TEMPERATURE" = "x01" ]; then
	...
	REDGAIN=" - Red Gain: $(ddcutil --sleep-multiplier=$SLEEPMULTIPLIER -t --bus=$BUS getvcp 16 | awk -F' ' '{print $4}')"
	GREENGAIN=" - Green Gain: $(ddcutil --sleep-multiplier=$SLEEPMULTIPLIER -t --bus=$BUS getvcp 18 | awk -F' ' '{print $4}')"
	BLUEGAIN=" - Blue Gain: $(ddcutil --sleep-multiplier=$SLEEPMULTIPLIER -t --bus=$BUS getvcp 1a | awk -F' ' '{print $4}')"
	VOLUME="Speaker Volume: $(ddcutil --sleep-multiplier=$SLEEPMULTIPLIER -t --bus=$BUS getvcp 62 | awk -F' ' '{print $4}')"
	INFO="Resolution:
$HORIZONTALFREQ
$VERTICALFREQ
$INPUT
...
info_advanced_menu(){
	...
	DCT="Display Controller Type: $(ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS getvcp c8 | awk -F'): ' '{print $2}')"
	FIRMWARE="Display Firmware Level: $(ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS getvcp c9 | awk -F'): ' '{print $2}')"
	VCP="VCP Version: $(ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS getvcp df | awk -F'): ' '{print $2}')"
	INFOADVANCED="$SERIAL
$FPSBL
$DTT
$AEK
...
reset_menu(){
	while true; do
		...
		if [ "$reset_choice" -eq 1 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 04 03
		elif [ "$reset_choice" -eq 2 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 05 03
		elif [ "$reset_choice" -eq 3 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 06 03
		elif [ "$reset_choice" -eq 4 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 08 03
		elif [ "$reset_choice" -eq 5 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp 0a 03
...
reset_restore_menu(){
	while true; do
		...
		reset_choice=$?
		if [ "$reset_choice" -eq 1 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp b0 01
		elif [ "$reset_choice" -eq 2 ]; then
			ddcutil --sleep-multiplier=$SLEEPMULTIPLIER --bus=$BUS setvcp b0 02
		elif [ "$reset_choice" -eq 98 ]; then
			reset_menu
		else
			exit 0
